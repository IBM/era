...
void do_recv_pipeline(int num_recvd_vals, float* recvd_in_real, size_t recvd_in_real_sz, 
	...
	void * T1 = __hetero_task_begin(26,
																	...
																	3, 
																	input_data_arg, input_data_arg_sz,
																	recvd_msg_len, recvd_msg_len_sz, recvd_msg, recvd_msg_sz, "recv_pipeline_impl_task");
#endif
...
//#if defined(HPVM) && true
//			void * T0 = __hetero_task_begin(2, num_inputs, input_data_arg, input_data_arg_sz, 0, "logging_task");
//#endif
//			{
...
//			DO_NUM_IOS_ANALYSIS(printf("Calling delay IN %u OUT %u\n", num_inputs, num_del16_vals));
			...
			/************ TODO: Uncomment me (and the stuff above me) 
...
********************/
...
#endif
		{
			...
			void * T9 = __hetero_task_begin(7, correlation_complex_arg, correlation_complex_arg_sz, 
																			...
																			num_sync_short_vals, num_sync_short_vals_sz, 
																			delay16_out, DELAY_16_MAX_OUT_SIZE, num_inputs,
...
#endif
			************************* TODO: Uncomment me *************************/
#if defined(HPVM) // TODO Remove the task below
			void * T_end = __hetero_task_begin(23,
                 num_inputs,
                 out_msg_len, out_msg_len_sz,
                 out_msg, out_msg_sz,
                 // Local variables used by compute()
                 scrambled_msg, scrambled_msg_sz,
                 ss_freq_offset, ss_freq_offset_sz,
                 ...
                 3,
                 input_data_arg, input_data_arg_sz,
                 out_msg_len, out_msg_len_sz, out_msg, out_msg_sz, "hello_world_task");
			input_data_arg[0] = input_data_arg[10];
			*out_msg_len = 5;
			out_msg[0] = out_msg[1];
			printf("hi");
			__hetero_task_end(T_end);
			__hetero_section_end(Section);
#endif
