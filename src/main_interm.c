...
void fuse_maps(int* n_recvd_in, size_t n_recvd_in_sz,
								...
								/* Return by argument variables:
									 ...
									 uncmp_data, uncmp_data_sz, dec_bytes, dec_bytes_sz */) {
													 ...
													 void * T1 = __hetero_task_begin(17, n_recvd_in, n_recvd_in_sz, recvd_in_real, recvd_in_real_sz,
													 ...
													 *dec_bytes = LZ4_decompress_safe((char * ) recvd_msg, (char * ) uncmp_data, *recvd_msg_len,
													 ...
													 void * T3 = __hetero_task_begin(3, uncmp_data, uncmp_data_sz, dec_bytes, dec_bytes_sz,
																					 observations, observations_sz, 1, observations, observations_sz);
#endif
...
#if defined(HPVM) && true
													 void * T4 = __hetero_task_begin(2, observations, observations_sz, uncmp_data, uncmp_data_sz, 1, observations, observations_sz);
#endif
...
#if defined(HPVM) && true
												// 20 inputs, 7 outputs
												void * LaunchInner = __hetero_launch((void*) fuse_maps, 20, n_recvd_in, n_recvd_in_sz,
																				recvd_in_real, recvd_in_real_sz, recvd_in_imag, recvd_in_imag_sz,
...
#define ofdm_max_out_size 33280 // from xmit_pipe.c
				...
				__hetero_wait(LaunchInner);
#else
				do_xmit_pipeline_wrapper(n_cmp_bytes, n_cmp_bytes_sz, (char*) cmp_data, cmp_data_sz,
												&n_xmit_out, sizeof(int), xmit_out_real, xmit_out_real_sz,
												xmit_out_imag, xmit_out_imag_sz,
												/* Start of local variables for do_xmit_pipeline*/
												&psdu_len, psdu_len_sz, pckt_hdr_out, pckt_hdr_out_sz,
...
int main(int argc, char * argv[]) {
				...
				while ((!hit_eof) && (lidar_count < max_time_steps)) {
								...
								if (l_buffer[0] == 'L' && l_buffer[9] == 'L') {
												...
												// Arguments to lidar_root; used for return by argument within lidar_root
												int n_cmp_bytes = 0;
...
#if defined(HPVM) && true
												// Merge lidar_root and encode_transmit_occgrid
