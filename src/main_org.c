...
void process_lidar_to_occgrid(lidar_inputs_t * lidar_inputs) {
  ...
  void * T2 = __hetero_task_begin(1, observations, observation_sz, 1, observations, observation_sz);
...
#endif
  int n_cmp_bytes = LZ4_compress_default((char *) local_map, (char *) cmp_data,
...
void * lidar_root(void * lidar_inputs, size_t lidarin_sz, void * lidar_outputs, size_t lidarout_sz) {
    void * Section = __hetero_section_begin();
    void * T1 = __hetero_task_begin(1, lidar_inputs, lidarin_sz, 1, lidar_outputs, lidarout_sz);

    process_lidar_to_occgrid(lidar_inputs); // buffer, total_bytes_read);
...
void * cv_root(label_t tr_val, label_t * out_label, size_t outlabel_sz) {
    void * Section = __hetero_section_begin();
    void * T1 = __hetero_task_begin(1, tr_val, 1, out_label, outlabel_sz);
...
int main(int argc, char * argv[]) {
  ...
  while ((!hit_eof) && (lidar_count < max_time_steps)) {
    ...
    if (l_buffer[0] == 'L' && l_buffer[9] == 'L') {
      ...
      //#endif
      void * dfg = __hetero__launch((void *) lidar_root, 1, (void *) (&lidar_inputs), sizeof(lidar_inputs_t),
